...
def talos_patches(value: str) -> list[str]:
    path = Path(f'bootstrap/templates/talos/patches/{value}')
    if not path.is_dir():
        return []
...
class Plugin(makejinja.plugin.Plugin):
    ...
    def data(self) -> makejinja.plugin.Data:
        ...
        data.setdefault('bootstrap_node_default_gateway', nthhost(data.get('bootstrap_node_cidr'), 1))
        data.setdefault('bootstrap_node_dns_servers', ['1.1.1.1', '1.0.0.1'])
        data.setdefault('bootstrap_node_ntp_servers', ['162.159.200.1', '162.159.200.123'])
        data.setdefault('bootstrap_cluster_cidr', '10.42.0.0/16')
        data.setdefault('bootstrap_service_cidr', '10.43.0.0/16')
        data.setdefault('bootstrap_github_repository_branch', 'main')
        data.setdefault('bootstrap_github_repository_visibility', 'public')
